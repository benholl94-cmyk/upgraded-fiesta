...
# Module-level constant (not a mutable class attribute) so no test can
# accidentally mutate a shared instance and leak state into another test.
_VALID_HTML_SNIPPETS = {
    ...
    def test_each_required_needle_in_valid_snippet(self):
        for needle in REQUIRED_HTML:
            with self.subTest(needle=needle):
                self.assertIn(needle, _VALID_HTML_SNIPPETS[needle])
