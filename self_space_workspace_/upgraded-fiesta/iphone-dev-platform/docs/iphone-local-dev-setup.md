...
- **Requests & Updates**: vollständige Änderungsanfragen mit Ziel, Umfang, Update-Plan, Sicherheitsgrenzen und Abnahme erzeugen.
- **Deploy**: statische Dateien ohne Build-Schritt veröffentlichen.

Für sauberen Zugriff gelten drei Regeln: keine Secrets in Projektdateien, keine blinden Remote-Ausführungen und Direct-Inject-Blöcke immer vor dem Kopieren lesen. Projektnamen werden auf sichere Zeichen begrenzt und der generierte Startblock schreibt nur unter `~/Developer/scratch/`. Request-Pakete sind lokale Markdown-Vorlagen und werden erst kopiert oder weitergegeben, wenn du sie geprüft hast.

Zur Initialprüfung des Repositories gibt es einen reproduzierbaren lokalen Einstieg:
...
Die Prüfungen benötigen keine Installation von npm-Paketen. Sie validieren statische Dateien, wichtige Zugänge, Guide-Struktur, Manifest und JavaScript-Syntax.

## 5. 15-Minuten-Schnellstart

Dieser Schnellstart erzeugt einen funktionierenden End-to-End-Workflow, bevor du dich mit allen Details beschäftigst. Wenn du zuerst die autonome Plattform nutzen willst, starte `npm run serve` oder einen statischen Hoster, wähle ein Profil und kopiere den generierten Direct-Inject-Block. Die UI kann auch direkt per `index.html` geöffnet werden; für die vollständige QA-Prüfung muss der Guide aber ladbar sein.
