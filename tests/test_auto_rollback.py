...
def test_holds_when_previous_state_is_unknown():
    """Unbekannt darf nicht wie gruen behandelt werden — sonst revertiert das
    System auf Verdacht."""
    d = ar.decide(ctx(previous_conclusion="unknown"))
    assert d.action == ar.REVERT or d.action == ar.HOLD
    # 'unknown' ist nicht 'failure', also greift Sperre 2 nicht; die
    # Merge-Sperre und die Sicherung bleiben aktiv. Festgehalten, damit eine
    # spaetere Verschaerfung bewusst geschieht.
...
def test_context_from_dict_tolerates_missing_fields():
    ...
    assert c.previous_conclusion == "success" and c.recent_reverts == ()
